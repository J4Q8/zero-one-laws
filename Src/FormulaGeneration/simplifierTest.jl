...
@testset verbose = true "Simplifier tests" begin
    ...
    @testset "multiple bimplications" begin
        ...
        @test testS("(T↔p)↔(T↔T)", "T↔p")
        @test testS("(F↔p)↔(F↔F)", "p↔F")
        @test testS("(T↔p)↔(F↔T)", "p↔F")
        @test testS("(T↔p)↔(T↔~p)", "F")
        @test testS("(F↔F)↔(F↔~T)", "T")
        @test testS("(F↔p)↔(r↔p)", "r↔F")
        @test testS("(T↔p)↔(r↔(p↔(F↔T)))", "r↔F")
        @test testS("(F↔p)↔(r↔(p↔(q↔F)))", "q↔(r↔T)")
        @test testS("(~q↔p)↔(r↔(p↔(q↔T)))", "r↔F")
    end 

    @testset "complex simplifications" begin
        @test testS("(~q^p)^(r^(p^(q-T)))", "((p^~q)^r)")
        @test testS("◻ ( ◻ ⊤ ∧ ◻ ( ◻ ( q → p ) ↔ ( ◻ ⊥ ∧ ( ⊥ ∧ ⊤ ) ) ) )", "◻ ( ◻ ( ◻ ( q → p ) ↔ ( F ) ) )")
        @test testS("(p->q)^~(p->q)" ,"F")
        @test testS("((p->q)^~(p->q))V((p=p)->q)" ,"T->q")
        @test testS("(◻ ⊤ ∧ (( ◇ F) ^p))->(p^~p)" ,"T")
        @test testS("(~◻ F ∧ ~◇(( ◇ F) ^p))->(p^~p)" ," ◇ ⊤ → ⊥")
